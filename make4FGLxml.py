...
class srcList:
	...
	def __init__(self,sources,ft1,out='mymodel.xml'):
		if not fileCheck(sources): #check that file exists
			print(("Error:  {0} not found.").format(sources))
			return
		if fileCheck(out):
			print(('Warning: {0} already exists, file will be overwritten if you proceed with makeModel.').format(out))
		self.srcs=sources
		self.out=out
		self.roi=getPos(ft1)
	...
	def Print(self):
		...
		print(('Selecting {0} degrees around (ra,dec)=({1},{2})').format(self.roi[2],self.ra,self.dec))
...
def addSrcsFITS(sL,GD,GDn,ISO,ISOn,oldNames):
	...
	try:
		...
		if sL.var==True:
			print(("Error: requested to set variables sources free but 'Variability_Index' not found in {0}").format(sL.srcs))
			...
			if (dist<x and dist>=x-step) or (x==sL.roi[2]+10. and dist==x):
				...
				if E and not sL.psF:
					...
					if efunc=='SpatialMap':
					  ...
					  skydir+='\t</spatialModel>\n'
					  print(('Extended source {0} in ROI with %s spatial model.').format(En,efunc))
				else:
